...
if defined NEEDS_INSTALL (
    echo [DictarApp] Instalando/actualizando dependencias ^(puede tardar la primera vez^)...
    "%PY%" -m pip install -r requirements.txt
    if errorlevel 1 (
        echo.
        echo ERROR: fallo la instalacion de dependencias.
        echo Probablemente falta Microsoft C++ Build Tools para compilar pyaudio.
        echo La app igual funciona con sounddevice como fallback ^(ver README^).
        pause
        exit /b 1
    )
